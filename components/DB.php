...
class DB
{
	...
	*/
	...
	/**
	 *  @brief Конструктор, в котором подключаемся к БД, а так же ловим исключения и пишем в лог в случае ошибки
	 *
	 *  @return void
	 */
	...
	/**
	 *  @brief Метод для создания объекта текущего класса DB
	 *
	...
	/**
	 *  @brief Метод безопасного запроса к БД с использованием индексированых плейсхолдеров
	...
	/**
 	 * @brief Метод для получения выборки из БД
 	 *
	...
	*/
	...
	/**
 	 * @brief Метод для обновления данных в БД
 	 *
 	 * @param string $table Принимает строку -  наименование таблицы без префикса
	...
	*/
	...
	/**
 	 * @brief Метод для удаления данных из БД
 	 *
 	 * @param string $table Принимает строку -  наименование таблицы без префикса
 	 * @param array $where Принимает одномерный или двумерный массив для формирования предиката ([['id'=>'='],['AND', 'OR']])
 	 * @param array|scalar $params Принимает массив или скалярное значение параметров для sql-запроса
	...
	*/
	...
	/**
 	 * @brief Закрытый метод добавляет апострофы к именам столбцов и переводит в строку
 	 *
 	 * @param array|string $column Ппринимает массив или строку наименования столбцов
	...
	*/
	...
	/**
 	 * @brief Закртытый метод для получения отформатированного предиката where
 	 *
 	 * @param array $where Принимает одномерный или двумерный массив для формирования предиката ([['id'=>'='],['AND', 'OR']])
	...
	*/
	...
	/**
 	 * @brief Закрытый метод для получения отформатированной строки order
 	 *
	...
	*/
	...
	/**
 	 * @brief Закрытый метод для получения отформатированной строки limit
 	 *
	...
	/**
 	 * @brief Закрытый метод, который формирует строку обновляемых данных
 	 *
 	 * @param string|array $data Принимает строку или массив полей, которые нунжно обновить
	...
	*/
	...
	/**
	 *  @brief Деструктор, в котором производим отключение от БД
	 *
	 *  @return void
	 */
